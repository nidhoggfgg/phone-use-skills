...
先列出现有设备。没有目标设备时使用用户提供的手机地址配对；地址未知或多台设备无法区分时向用户询问。`pair` 在 stderr 显示核对码并等待手机批准，告诉用户核对码及对应设备，在批准期间保持该进程；不要反复启动配对。超时、拒绝或暂停需要解决实际原因后再继续。手机必须启动服务并开启无障碍，执行输入时需亮屏解锁。
